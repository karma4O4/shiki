...
async function run() {
  ...
  await fs.rm(new URL("./compares", import.meta.url), {
    recursive: true,
    force: true,
  });
  ...
  for (const lang of Object.keys(bundledLanguages)) {
    ...
    const grammars = shikiWasm
      .getLoadedLanguages()
      .map((l) => shikiWasm.getLanguage(l));
    const patterns = new Set<string>();
    grammars.map((grammar: any) =>
      getPatternsOfGrammar(grammar._grammar, patterns),
    );
    ...
    highlightDiff =
      highlightB && highlightA !== highlightB
    ...
    if (highlightB && highlightMatch !== true) {
      ...
      await fs.mkdir(new URL("./compares", import.meta.url), {
        recursive: true,
      });
  ...
  report.sort((a, b) => {
    ...
    return (
      a.patternsFailed.length - b.patternsFailed.length ||
  ...
  function createTable(report: ReportItem[]) {
    const table: readonly [string, string, string, string, string][] = [
      ...
      ["---", ":---", "---:", "---:", "---:"],
      ...report.map((item) => {
        const diffChars = item.diff
          .map((diff) => (diff[0] === 1 ? diff[1].length : 0))
          .reduce((a, b) => a + b, 0);
        return [
          item.lang,
          item.highlightMatch === true
            ? "✅ OK"
            : item.highlightMatch === "error"
              ? "❌ Error"
              : `[🚧 Mismatch](https://textmate-grammars-themes.netlify.app/?grammar=${item.lang})`,
          item.patternsParsable === 0 ? "-" : item.patternsParsable.toString(),
          item.patternsFailed.length === 0
            ? "-"
            : item.patternsFailed.length.toString(),
  ...
  const reportOk = report.filter(
    (item) => item.highlightMatch === true && item.patternsFailed.length === 0,
  );
  const reportMismatch = report.filter(
    (item) => item.highlightMatch === false && item.patternsFailed.length === 0,
  );
  const reportError = report.filter(
    (item) => item.highlightMatch === "error" || item.patternsFailed.length > 0,
  );
...
function serializeTokens(
  ...
  const tokens = shiki
    ...
    .join("\n");
  const html = shiki.codeToHtml(sample, {
    lang: lang as any,
    theme: "vitesse-dark",
  });
  return {
