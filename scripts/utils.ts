export function traverseGrammarPatterns(
  a: any,
  ...
  if (a.foldingStartMarker) {
    ...
    if (pattern != null) a.foldingStartMarker = pattern;
  }
  if (a.foldingStopMarker) {
    const pattern = callback(a.foldingStopMarker);
    if (pattern != null) a.foldingStopMarker = pattern;
  }
  if (a.firstLineMatch) {
    const pattern = callback(a.firstLineMatch);
    if (pattern != null) a.firstLineMatch = pattern;
  }
  if (a.match) {
    const pattern = callback(a.match);
    if (pattern != null) a.match = pattern;
  }
  if (a.begin) {
    const pattern = callback(a.begin);
    if (pattern != null) a.begin = pattern;
  }
  if (a.end) {
    const pattern = callback(a.end);
    if (pattern != null) a.end = pattern;
  }
  if (a.while) {
    const pattern = callback(a.while);
    if (pattern != null) a.while = pattern;
  }
  if (a.patterns) {
