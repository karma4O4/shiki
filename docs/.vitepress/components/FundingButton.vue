...
<template>
  <Tooltip
    ...
    align-middle
  >
    <button title="Funding" hover="bg-gray/10" p1 rounded>
      <div
        i-carbon:favorite-filled
        op25
        group-hover:op100
        group-focus-within:op100
    ...
    <template #popper>
      <div p2 class="vp-doc text-sm">
        ...
        <div v-for="(link, i) in funding" :key="i" text-nowrap>
          <template v-if="link.handle"> {{ link.name }}: </template>
