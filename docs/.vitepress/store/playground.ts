...
export const usePlayground = defineStore("playground", () => {
  const lang = useLocalStorage<BundledLanguage>(
    "shiki-playground-lang",
    "typescript",
  );
  const theme = useLocalStorage<BundledTheme>(
    "shiki-playground-theme",
    "vitesse-dark",
  );
  ...
  function randomize(): void {
    if (allLanguages.value.length && allThemes.value.length) {
      lang.value = allLanguages.value[
        Math.floor(Math.random() * allLanguages.value.length)
      ].name as BundledLanguage;
      theme.value = allThemes.value[
        Math.floor(Math.random() * allThemes.value.length)
      ].name as BundledTheme;
    }
  }
  ...
  (async () => {
    ...
    const webGrammars = allGrammars.grammars.filter((grammar) =>
      grammar.categories?.includes("web"),
    );
    ...
    function fetchSample(id: string): Promise<string | undefined> {
      if (!samplesCache.has(id)) {
        samplesCache.set(
          id,
          fetch(
            `https://raw.githubusercontent.com/shikijs/textmate-grammars-themes/main/samples/${id}.sample`,
          )
            .then((r) => r.text())
            .catch((e) => {
              console.error(e);
