...
const report = (await fs
  .readFile(
    new URL("../../scripts/report-engine-js-compat.json", import.meta.url),
    "utf-8",
  )
  .then(JSON.parse)) as ReportItem[];
const langs = report
  .filter((i) => i.highlightMatch === true)
  .map((i) => i.lang)
  .slice(...RANGE) as BundledLanguage[];
// Clone https://github.com/shikijs/textmate-grammars-themes to `../tm-grammars-themes`
const samples = await Promise.all(
  langs.map((lang) =>
    fs.readFile(
      new URL(
        `../../tm-grammars-themes/samples/${lang}.sample`,
        import.meta.url,
      ),
      "utf-8",
    ),
...
const shikiJsPrecompiled = await createHighlighter({
  langs: await Promise.all(
    langs.map((lang) => import(`@shikijs/langs-precompiled/${lang}`)),
  ),
  themes: ["vitesse-dark"],
  engine: jsRaw,
});
...
for (const lang of langs) {
  describe(lang, () => {
    ...
    bench("js-precompiled", () => {
      shikiJsPrecompiled.codeToTokensBase(code, {
        lang,
        theme: "vitesse-dark",
      });
    });
