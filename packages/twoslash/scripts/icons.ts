...
async function buildIcons(
  ...
  const result = Object.fromEntries(
    Object.entries(map).map(([key, value]) => {
      ...
      if (!icon) throw new Error(`Icon not found: ${value}`);
  ...
  await fs.writeFile(
    ...
    `${JSON.stringify(
      result,
      (r, v) => {
        if (v?.position) delete v.position;
        return v;
      },
      2,
    )}\n`,
...
await buildIcons("./src/icons-completions.json", {
...
await buildIcons("./src/icons-tags.json", {
