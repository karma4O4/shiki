...
`.trim();
  ...
  await expect(styleTag + htmlMain).toMatchFileSnapshot(
    "./out/includes/main.html",
  );
...
`;
  ...
  await expect(styleTag + htmlA).toMatchFileSnapshot(
    "./out/includes/nested_includes-a.html",
  );
  ...
  await expect(styleTag + htmlB).toMatchFileSnapshot(
    "./out/includes/nested_includes-b.html",
  );

  const htmlC = await codeToHtml(c, { ...options });

  await expect(styleTag + htmlC).toMatchFileSnapshot(
    "./out/includes/nested_includes-c.html",
  );
});
