                  ...
                  ...(tag[1]
                    ? [
                        <Element>{
                          ...
                          children: renderMarkdownInline.call(
                            this,
                            tag[1],
                            `tag:${tag[0]}`,
                          ),
  ...
  return {
    nodeStaticInfo(info, node) {
      ...
      const popup = extend(hast?.hoverPopup, {
        ...
        tagName: "span",
        properties: {
          class: ["twoslash-popup-container", classExtra]
            .filter(Boolean)
            .join(" "),
        },
    ...
    nodeQuery(query, node) {
      ...
      return extend(hast?.queryToken, {
        ...
        properties: {
          ...
          class:
            "twoslash-hover twoslash-query-persisted twoslash-query-presisted",
        },
    ...
    nodeCompletion(query, node) {
      ...
      const cursor = extend(hast?.completionCursor, {
        ...
        tagName: "span",
        properties: {
          class: ["twoslash-completion-cursor", classExtra]
            .filter(Boolean)
            .join(" "),
        },
        children: [],
      ...
      const popup = extend(hast?.completionPopup, {
        ...
        tagName: "ul",
        properties: {
          class: ["twoslash-completion-list", classExtra]
            .filter(Boolean)
            .join(" "),
        },
        children: items,
      ...
      if (errorRendering !== "hover") {
        return [
          extend(hast?.errorToken, {
            ...
            tagName: "span",
            properties: {
              class: [`twoslash-error`, getErrorLevelClass(error)]
                .filter(Boolean)
                .join(" "),
            },
            children,
    ...
    lineQuery(query, node) {
      ...
      const themedContent = highlightPopupContent.call(this, query);
      const targetNode =
        node?.type === "element" ? node.children[0] : undefined;
      const targetText = targetNode?.type === "text" ? targetNode.value : "";
      const offset = Math.max(
        0,
        (query.character || 0) + Math.floor(targetText.length / 2) - 2,
      );
      ...
      return [
        {
          ...
          tagName: "div",
          properties: {
            class: ["twoslash-meta-line twoslash-query-line", classExtra]
              .filter(Boolean)
              .join(" "),
          },
          children: [
      ...
      return [
        {
          ...
          properties: {
            class: [
              `twoslash-tag-line twoslash-tag-${tag.name}-line`,
              classExtra,
            ]
              .filter(Boolean)
              .join(" "),
