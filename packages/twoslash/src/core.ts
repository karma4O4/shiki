...
// internal
type _CacheableTwoslashShikiFunction = (
  code: string,
  lang?: string,
...
export function createTransformerFactory(
  ...
  return function transformerTwoslash(
      ...
      filter = (lang, _, options) => {
        return (
          ...
          !disableTriggers.some((i) =>
            typeof i === "string"
              ? options.meta?.__raw?.includes(i)
              : i.test(options.meta?.__raw || ""),
          )
        );
      },
    } = options;
    ...
    if (typesCache) {
      _twoslasher = ((
        ...
        const preprocessed = typesCache?.preprocess?.(
          code,
          lang,
          options,
          meta,
        );
        if (preprocessed !== undefined) code = preprocessed;
        ...
        return twoslash;
      }) satisfies _CacheableTwoslashShikiFunction &
        TwoslashShikiFunction &
      ...
      preprocess(code) {
        ...
        if (filter(lang, code, this.options)) {
          try {
            ...
            const twoslash = (_twoslasher as _CacheableTwoslashShikiFunction)(
              codeWithIncludes,
              lang,
              twoslashOptions,
              this.meta,
      ...
      tokens(tokens) {
        ...
        return splitTokens(
          ...
          twoslash.nodes.flatMap((i) =>
            ["hover", "error", "query", "highlight", "completion"].includes(
              i.type,
            )
      ...
      code(codeEl) {
        ...
        const insertAfterLine = (
          ...
          if (line >= this.lines.length) {
            ...
            if (index === -1) {
              onShikiError(
                new ShikiTwoslashError(
                  `Cannot find line ${line} in code element`,
                ),
                this.source,
                this.options.lang,
              );
        ...
        for (const node of twoslash.nodes) {
          if (node.type === "tag") {
            if (renderer.lineCustomTag)
              insertAfterLine(
                node.line,
                renderer.lineCustomTag.call(this, node),
              );
            continue;
          }

          const tokens = locateTextTokens(
            node.line,
            node.character,
            node.length,
          );
          ...
          if (
            !tokens.length &&
            !(node.type === "error" && renderer.nodesError)
          ) {
            onShikiError(
              new ShikiTwoslashError(
                `Cannot find tokens for node: ${JSON.stringify(node)}`,
              ),
              this.source,
              this.options.lang,
            );
          ...
          const wrapTokens = (
            fn: (children: ElementContent[]) => ElementContent[],
          ): void => {
          ...
          switch (node.type) {
            case "error": {
              if (renderer.nodeError) {
                tokens.forEach((token) => {
                  ...
                  Object.assign(
                    token,
                    renderer.nodeError!.call(this, node, clone),
                  );
                });
              }
              if (renderer.nodesError) {
                ...
                actionsHighlights.push(() => {
                  wrapTokens(
                    (targets) =>
                      renderer.nodesError?.call(this, node, targets) || targets,
                  );
                });
              }
            ...
            case "query": {
              ...
              if (token && renderer.nodeQuery) {
                ...
                Object.assign(
                  token,
                  renderer.nodeQuery!.call(this, node, clone),
                );
              }
              if (renderer.lineQuery)
                insertAfterLine(
                  node.line,
                  renderer.lineQuery.call(this, node, token),
                );
              break;
            }
            case "completion": {
              if (renderer.nodeCompletion) {
                tokens.forEach((token) => {
                  ...
                  Object.assign(
                    token,
                    renderer.nodeCompletion!.call(this, node, clone),
                  );
                });
              }
              if (renderer.lineCompletion)
                insertAfterLine(
                  node.line,
                  renderer.lineCompletion.call(this, node),
                );
              break;
            }
            case "highlight": {
              if (renderer.nodesHighlight) {
                actionsHighlights.push(() => {
                  wrapTokens(
                    (targets) =>
                      renderer.nodesHighlight?.call(this, node, targets) ||
                      targets,
                  );
                });
              }
