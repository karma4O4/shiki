...
describe("explicitTrigger", async () => {
  ...
  it.each(validMetaStrings)(
    'should colorize brackets for meta string "%s"',
    (meta) => {
      ...
      expect(
        ...
        '<span style="color:Y">[</span><span style="color:Y">]</span>',
      );
    },
  );
  ...
  it.each(invalidMetaStrings)(
    'should not colorize brackets for meta string "%s"',
    (meta) => {
      const code = "let values: number[] = [1, 2, 3];";
      expect(
        highlighter.codeToHtml(code, {
