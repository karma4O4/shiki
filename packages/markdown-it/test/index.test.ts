...
it("run for base", { timeout: 10_000 }, async () => {
  ...
  const result = md.render(
    await fs.readFile(new URL("./fixtures/a.md", import.meta.url), "utf-8"),
  );
...
it("run for fallback language", { timeout: 10_000 }, async () => {
  ...
  const result = md.render(
    await fs.readFile(new URL("./fixtures/b.md", import.meta.url), "utf-8"),
  );
...
it("run for default language", { timeout: 10_000 }, async () => {
  ...
  const result = md.render(
    await fs.readFile(new URL("./fixtures/c.md", import.meta.url), "utf-8"),
  );
...
it("lang-alias", async () => {
  ...
  const result = md.render(
    await fs.readFile(
      new URL("./fixtures/lang-alias.md", import.meta.url),
      "utf-8",
    ),
  );
