...
export async function loadLangs() {
  ...
  for (const file of allLangFiles) {
    ...
    const lang =
      grammars.find((i) => i.name === content.name) ||
      injections.find((i) => i.name === content.name);
    if (!lang) {
    ...
    if (LANGS_LAZY_EMBEDDED_ALL[lang.name]) {
      ...
      json.embeddedLangsLazy =
        (json.embeddedLangs || []).filter((i) => !includes.includes(i)) || [];
...
export async function prepareLangs() {
  ...
  for (const json of resolvedLangs) {
    ...
    if (deps.length > 10) console.log(json.name, json.embeddedLangs);

    const depsStr =
      [...deps.map((i) => `...${i.replace(/\W/g, "_")}`), "lang"].join(",\n") ||
      "";
...
`
    ...
    for (const alias of json.aliases || []) {
      if (isInvalidFilename(alias)) continue;
      await fs.writeFile(
        `./dist/${alias}.mjs`,
...
`,
    ...
    for (const name of [...(json.aliases || []), json.name]) {
      if (isInvalidFilename(name)) continue;
...
${exportedFileNames
  .filter((i) => !resolvedLangs.some((j) => j.name === i))
  .sort()
...
`,
  ...
  packageJson.exports = {
    ".": "./dist/index.mjs",
    ...Object.fromEntries(
      exportedFileNames.map((i) => [`./${i}`, `./dist/${i}.mjs`]),
    ),
  };
  await fs.writeFile(
    "./package.json",
  ...
  async function writeLanguageBundleIndex(fileName: string, ids: string[]) {
    ...
    const bundled = Array.from(bundledIds)
      .map((id) => grammars.find((i) => i.name === id)!)
      .filter(Boolean);
...
export const bundledLanguagesInfo: BundledLanguageInfo[] = ${JSON.stringify(
        info,
        null,
        2,
      )
...
`,
  ...
  await writeLanguageBundleIndex("langs-bundle-web", [
