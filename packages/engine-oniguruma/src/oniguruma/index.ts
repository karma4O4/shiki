...
function throwLastOnigError(onigBinding: IOnigBinding): void {
  throw new ShikiError(
    onigBinding.UTF8ToString(onigBinding.getLastOnigError()),
  );
}
...
export class OnigScanner implements IOnigScanner {
  ...
  private _findNextMatchSync(
    ...
    // else
    const resultPtr = onigBinding.findNextOnigScannerMatch(
      this._ptr,
      string.id,
      string.ptr,
      string.utf8Length,
...
function isInstantiatorOptionsObject(
  dataOrOptions: any,
): dataOrOptions is { instantiator: WebAssemblyInstantiator } {
  return typeof dataOrOptions.instantiator === "function";
}

function isInstantiatorModule(
  dataOrOptions: any,
): dataOrOptions is { default: WebAssemblyInstantiator } {
  return typeof dataOrOptions.default === "function";
}

function isDataOptionsObject(
  dataOrOptions: any,
): dataOrOptions is { data: ArrayBufferView | ArrayBuffer | Response } {
  ...
  async function _load(): Promise<void> {
    onigBinding = await createOnigasm(async (info) => {
      ...
      if ("instance" in instance)
        instance = (instance as WebAssembly.WebAssemblyInstantiatedSource)
          .instance;
...
function _makeArrayBufferLoader(
  data: ArrayBufferView | ArrayBuffer | WebAssembly.Module,
): WebAssemblyInstantiator {
