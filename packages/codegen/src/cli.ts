...
cli
  ...
  .action(async (path, options) => {
    ...
    const langs = (
      (Array.isArray(options.langs)
        ? options.langs.join(",")
        : options.langs || "") as string
    )
      .split(",")
      .map((lang) => lang.trim())
      .filter(Boolean);
    ...
    if (!themes.length) {
      throw new Error(
        "No themes specified, use --themes=theme-name to specify themes",
      );
    }
    if (!langs.length) {
      throw new Error(
        "No langs specified, use --langs=lang-name to specify langs",
      );
    }
