...
export async function codegen(
  options: ShikiCodegenOptions,
): Promise<ShikiCodegenResult> {
  const {
  ...
  const langs = options.langs.map((lang) => {
    const info = bundledLanguagesInfo.find(
      (i) => i.id === lang || i.aliases?.includes(lang),
    );
  ...
  if (typescript) {
    lines.push(
      "",
      `type BundledLanguage = ${langs
        .flatMap((lang) => [lang.id, ...(lang.aliases || [])])
        .map((lang) => `'${lang}'`)
        .join(" | ")}`,
  ...
  if (typescript) {
    lines.push(`export type { ${typeExports.sort().join(", ")} }`);
  }
