...
function createEngineWrapper(
  engine: RegexEngine,
): RegexEngine & { executions: Execution[] } {
    ...
    createScanner(patterns) {
      ...
      return {
        findNextMatchSync(
          ...
          const result = scanner.findNextMatchSync(
            string,
            startPosition,
            options,
          );
          executions.push({
            ...
            patterns,
            args: [
              typeof string === "string" ? string : string.content,
              startPosition,
              options,
            ],
            result,
...
describe("cases", async () => {
  ...
  for (const c of resolved) {
    ...
    run(c.c.name, async () => {
      const engineWasm = createEngineWrapper(await createOnigurumaEngine());
