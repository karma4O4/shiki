...
export default defineBuildConfig({
  ...
  hooks: {
    "rollup:done": async () => {
      ...
      const floatingVue = await fs.readFile(
        new URL("./node_modules/floating-vue/dist/style.css", import.meta.url),
        "utf-8",
      );
      const twoslash = await fs.readFile(
        new URL("./../twoslash/style-rich.css", import.meta.url),
        "utf-8",
      );
