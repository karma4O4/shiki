import type {
  MarkdownFenceSourceMap,
  MarkdownFencesSourceMapper,
} from "./markdown-fence";
import MarkdownIt from "markdown-it";

export const markdownItMapper: MarkdownFencesSourceMapper = function (
  code,
  path,
) {
...
function findNextNewLine(str: string, position: number): [number, number] {
  ...
  // CRLF case
  if (
    str.charCodeAt(idx) === 13 /* \r */ &&
    str.charCodeAt(idx + 1) === 10 /* \n */
  ) {
    return [idx, 2];
  }
  return [idx, 1];
