 ...
 */
export type MarkdownFencesSourceMapper = (
  code: string,
  path: string,
) => Map<number, MarkdownFenceSourceMap>;
...
export function createMarkdownFenceSourceCodec(
  mapper: MarkdownFencesSourceMapper,
): MarkdownFenceSourceMapCodec {
  const FENCE_SOURCE_WRAP = `<fsm-${Math.random().toString(36).slice(2)}>`;
  const FENCE_SOURCE_REGEX = new RegExp(
    `\/\/ ${FENCE_SOURCE_WRAP}(.+?)${FENCE_SOURCE_WRAP}\\n`,
  );
  ...
  function extractFromFence(code: string): {
    code: string;
    sourceMap: MarkdownFenceSourceMap | null;
  } {
