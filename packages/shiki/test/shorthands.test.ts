...
describe("should", () => {
  it("codeToHtml", async () => {
    expect(
      await codeToHtml('console.log("hello")', {
        lang: "js",
        theme: "vitesse-light",
      }),
    ...
    expect(
      await codeToHtml('<div class="foo">bar</div>', {
        lang: "html",
        theme: "min-dark",
      }),
    ).toMatchInlineSnapshot(
  ...
  it("codeToTokensBase", async () => {
    expect(
      await codeToTokensBase('console.log("hello")', {
        lang: "js",
        theme: "vitesse-dark",
  ...
  it("should allow subsequent valid calls after first invalid language", async () => {
    ...
    await expect(codeToHtml(code, { lang, theme })).rejects.toThrow(
      /Language `invalid` is not included in this bundle/,
    );
