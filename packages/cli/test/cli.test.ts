...
describe("isUrl", () => {
  it("valid URL", () => {
    ...
    expect(
      isUrl(
        "https://raw.githubusercontent.com/shikijs/shiki/refs/heads/main/taze.config.ts",
      ),
    ).toBe(true);
...
describe("readSource", () => {
  ...
  it("failed fetch", async () => {
    ...
    await expect(
      readSource("https://example.com/missing.js"),
    ).rejects.toThrowError(
      "Failed to fetch https://example.com/missing.js: 404 Not Found",
    );
...
describe("run", () => {
  ...
  it("remote URL", async () => {
    ...
    const output: string[] = [];
    await run(["node", "shiki", "https://example.com/code.ts"], (msg) =>
      output.push(msg),
    );
  ...
  it("--lang option", async () => {
    ...
    const output: string[] = [];
    await run(
      ["node", "shiki", "--lang", "python", "https://example.com/code"],
      (msg) => output.push(msg),
    );
