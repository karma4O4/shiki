...
export function transformerDecorations(): ShikiTransformer {
  const map = new WeakMap<
    ShikiTransformerContextMeta,
    TransformerDecorationsInternalContext
  >();

  function getContext(
    shiki: ShikiTransformerContextSource,
  ): TransformerDecorationsInternalContext {
    if (!map.has(shiki.meta)) {
      ...
      const decorations = (shiki.options.decorations || []).map(
        (d): ResolvedDecorationItem => ({
          ...d,
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    for (let j = i + 1; j < items.length; j++) {
      ...
      if (
        ...
        if (isFooHasBarStart && isFooHasBarEnd) continue; // nested
        if (isBarHasFooStart && isBarHasFooEnd) continue; // nested
        if (isBarHasFooStart && foo.start.offset === foo.end.offset) continue; // leading adjacent empty
        if (isFooHasBarEnd && bar.start.offset === bar.end.offset) continue; // trailing adjacent empty
        throw new ShikiError(
