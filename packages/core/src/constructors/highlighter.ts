...
import {
  codeToTokensBase,
  getLastGrammarState,
} from "../highlight/code-to-tokens-base";
...
export async function createHighlighterCore(
  ...
  return {
    getLastGrammarState: (...args: any[]) =>
      getLastGrammarState(internal, ...(args as [any]))!,
...
export function createHighlighterCoreSync(
  options: HighlighterCoreOptions<true>,
): HighlighterCore {
  const internal = createShikiInternalSync(options);

  return {
    getLastGrammarState: (...args: any[]) =>
      getLastGrammarState(internal, ...(args as [any, any])),
    codeToTokensBase: (code, options) =>
      codeToTokensBase(internal, code, options),
    codeToTokensWithThemes: (code, options) =>
      codeToTokensWithThemes(internal, code, options),
...
export const getSingletonHighlighterCore =
  /* @__PURE__ */ makeSingletonHighlighterCore(createHighlighterCore);
