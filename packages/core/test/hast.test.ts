...
describe("should", () => {
  it("works", async () => {
    ...
    expect(toHtml(hast)).toMatchInlineSnapshot(`
  ...
  it("structure inline", async () => {
    ...
    expect(toHtml(hast)).toMatchInlineSnapshot(
      `"<span style="color:#B07D48">console</span><span style="color:#999999">.</span><span style="color:#B07D48">log</span><br><span style="color:#B07D48">foo</span><span style="color:#999999">.</span><span style="color:#B07D48">bar</span>"`,
    );
  });
});
    ...
    transformers: [
      {
        ...
        span(node, line, col, parent) {
          node.children.forEach((child) => {
            if (
              child.type === "text" &&
              child.value.includes("[!code focus]")
            ) {
...
describe("merge same style", () => {
  it("merges adjacent tokens with same style", async () => {
    ...
    expect(html).toMatchInlineSnapshot(
      `"<pre class="shiki min-light" style="background-color:#ffffff;color:#24292eff" tabindex="0"><code><span class="line"><span style="color:#D32F2F">name:</span><span style="color:#22863A"> CI</span></span></code></pre>"`,
    );
  });
  ...
  it("merges adjacent tokens with the same dual themes", async () => {
    ...
    expect(html).toMatchInlineSnapshot(
      `"<pre class="shiki shiki-themes min-light min-light" style="background-color:#ffffff;--shiki-dark-bg:#ffffff;color:#24292eff;--shiki-dark:#24292eff" tabindex="0"><code><span class="line"><span style="color:#D32F2F;--shiki-dark:#D32F2F">name:</span><span style="color:#22863A;--shiki-dark:#22863A"> CI</span></span></code></pre>"`,
    );
  });
  ...
  it("does not merge tokens with decorations", async () => {
    ...
    expect(html).toMatchInlineSnapshot(
      `"<pre class="shiki min-light" style="background-color:#ffffff;color:#24292eff" tabindex="0"><code><span class="line"><span style="color:#D32F2F" class="highlighted-word">name</span><span style="color:#D32F2F">:</span><span style="color:#22863A"> CI</span></span></code></pre>"`,
    );
  });
});
