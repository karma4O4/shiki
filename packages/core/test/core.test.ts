...
describe("should", () => {
  it("works", async () => {
    ...
    expect(
      shiki.codeToHtml('console.log("Hi")', {
        lang: "javascript",
        theme: "nord",
      }),
  ...
  it("dynamic load theme and lang", async () => {
    ...
    expect(
      shiki.codeToHtml("print 1", { lang: "python", theme: "vitesse-light" }),
    ).toMatchInlineSnapshot(
      `"<pre class="shiki vitesse-light" style="background-color:#ffffff;color:#393a34" tabindex="0"><code><span class="line"><span style="color:#998418">print</span><span style="color:#2F798A"> 1</span></span></code></pre>"`,
    );
  });
  ...
  it("works without no initial langs and themes", async () => {
    ...
    const code = shiki.codeToHtml('console.log("Hi")', {
      lang: "javascript",
      theme: "nord",
    });
  ...
  it("works with alias", async () => {
    ...
    const code2 = shiki.codeToHtml('console.log("Hi")', {
      lang: "mylang2",
      theme: "nord",
    });
  ...
  it("works with alias override", async () => {
    ...
    const code = shiki.codeToHtml("const a: Foo = 1", {
      lang: "js",
      theme: "nord",
    });
    expect(code).toMatchInlineSnapshot(
  ...
  it("works with alias and special langs", async () => {
    ...
    const original1 = shiki.codeToHtml('console.log("Hi")', {
      lang: "text",
      theme: "nord",
    });
    const code1 = shiki.codeToHtml('console.log("Hi")', {
      lang: "lang1",
      theme: "nord",
    });
    ...
    const original2 = shiki.codeToHtml(ansiCode, {
      lang: "ansi",
      theme: "nord",
    });
...
describe("errors", () => {
  it("throw on invalid theme", async () => {
    ...
    await expect(() =>
      ...
      `[ShikiError: Theme \`invalid\` not found, you may need to load it first]`,
    );
  });
  ...
  it("throw on invalid lang", async () => {
    ...
    await expect(() =>
      shiki.codeToHtml('console.log("Hi")', { lang: "abc", theme: "nord" }),
    ).toThrowErrorMatchingInlineSnapshot(
      `[ShikiError: Language \`abc\` not found, you may need to load it first]`,
    );
  });
  ...
  it("throw on circular alias", async () => {
    ...
    await expect(() =>
      shiki.codeToHtml('console.log("Hi")', { lang: "mylang", theme: "nord" }),
    ).toThrowErrorMatchingInlineSnapshot(
      `[ShikiError: Circular alias \`mylang -> mylang2 -> mylang\`]`,
    );
  });
